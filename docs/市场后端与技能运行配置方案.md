...
- 从社区市场仓库加载技能包和工具包。当前工具包主要由 MCP Server 承载。
...
- 工具层当前通过 MCP 同时支持本地服务和第三方服务，通过 GitHub 配置发现。
...
- 本地工具承担需要本地环境、文件或私有网络的能力，当前主要通过 MCP 承载。
...
2. 模型：选择满足能力要求的模型。
3. 工具：安装、启动、填写密钥或参数。当前工具运行时主要由 MCP 承载。
...
- 市场项版本升级不能静默改变历史会话。
- 工具必须有明确权限边界，当前由 MCP Server 承载的工具实现也必须遵守同一套治理规则。
- 第三方发布必须先审核后上线。
...
4. 工具依赖检查使用 active 状态。当前由 MCP 承载的工具依赖不能只看配置是否存在。
