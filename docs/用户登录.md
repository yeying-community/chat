...
- [登录页前置方案评估（UCAN / Access Code）](./登录页前置方案评估.md)：在现有授权体系上增加前置登录页、路由守卫和过期跳转。
- [移动端登录与双后端鉴权问题分析与方案](./移动端登录与双后端鉴权方案.md)：解释移动端为什么不能直接复用 PC 端钱包登录，并给出分阶段方案。
- [Router 与 WebDAV 集成说明](./Router与WebDAV集成说明.md)：聚焦 Router / WebDAV 的请求链路、`audience`、代理边界和 CORS 约束。
- [UCAN / SIWE 权限设计（可扩展版）](./UCAN与SIWE权限设计可扩展版.md)：统一资源命名、动作模型、服务接入规范与迁移策略，作为后续能力扩展的基线文档。
- [新用户从首次使用到联系运营充值的完整流程](./用户使用流程.md)：从用户和运营视角说明钱包地址识别、试用额度与充值协同。
- [用户手册](./用户手册.md)：聚焦面具、对话、模型设置与日常使用说明。
...
| 中心化 UCAN | 登录页切换“中心化 UCAN（服务）”，地址 + TOTP 完成授权 | Router + WebDAV | 是，按后端分别签发（不依赖钱包插件） |
...
- **PC 端“钱包登录”是跨后端授权方式。**
- **中心化 UCAN 登录适用于无钱包插件场景（如移动端 / PC 调试强制服务签发）。**
- **中心化 UCAN 已支持 Router + WebDAV 双后端。** 前端会按目标后端的 `audience + capabilities` 分别申请短时 UCAN。
- **用户名/密码并不是统一登录。** 当前它只适用于 WebDAV Basic Auth。
- **Access Code / API Key 也不是统一登录。** 它主要作用于 Router / 模型请求。

### 2.1 登录路径强制开关（测试用）

前端支持通过环境变量强制登录路径，便于联调：

- `UCAN_LOGIN_FORCE_MODE=auto`：默认行为，优先钱包，失败后回退中心化授权。
- `UCAN_LOGIN_FORCE_MODE=wallet`：只走钱包插件，不回退中心化授权。
- `UCAN_LOGIN_FORCE_MODE=central`：只走中心化授权，跳过钱包连接。
...
- 声明里的目标服务：`service_hosts.router` 与 `service_hosts.webdav`
...
- Router 能力：`app:all:<appId> + invoke`
- WebDAV 能力：`app:all:<appId> + write`
- `appId` 由前端域名派生（例如 `localhost:3020 -> localhost-3020`）
...
- 登录兑换完成后，前端先拿到一张初始 UCAN（通常用于首次回调场景）。
- 后续访问 Router / WebDAV 时，前端会按目标后端自动调用 node issuer 接口，按 `audience + capabilities` 申请对应 UCAN。
- 因此不会再出现“把 Router 的 token 直接拿去调 WebDAV”导致的 `UCAN audience mismatch`。
...
中心化 UCAN 模式还会额外写入以下键（用于服务签发链路）：

- `ucanAuthMode`：当前授权模式（`wallet` / `central`）
- `centralAccessToken` / `centralAccessExpiresAt`：中心化登录 access token
- `centralAuthSubject`：当前授权地址
- `centralIssueSessionToken` / `centralIssueSessionExpiresAt`：node issuer 签发会话
- `centralUcanTokensV1`：按 `audience + capabilities` 缓存的多后端 UCAN 池
...
- Root 声明中的 `service_hosts` 缺失，或与当前 Router/WebDAV 地址不一致
