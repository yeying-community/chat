...
本文档是 [AI Native 能力分层架构](./AI-Native能力分层架构.md) 中存储层的现状梳理，重点说明应用数据、媒体文件、同步数据和后续知识库能力的存取边界。
...
3. `AppState`：指用于同步和导出的一整份应用状态快照，包含 `Chat`、`Access`、`Config`、`Skill`、`Prompt` 等 store 的非函数字段。
...
结构化业务状态必须按下面的 key 存储，后续新增 store 需要先补充这张基线，再落代码：

| Store | 当前 key | 职责 | 兼容旧 key |
| --- | --- | --- | --- |
| `Chat` | `chat-next-web-store` | 会话、消息、当前会话、删除标记 | 无 |
| `Access` | `access-control` | 接入控制、API 配置相关状态 | 无 |
| `Config` | `app-config` | 应用配置、模型默认配置、界面配置 | 无 |
| `Skill` | `skill-store` | 用户启用的技能、技能配置、技能工具声明 | `mask-store` |
| `Prompt` | `prompt-store` | 用户提示词 | 无 |
| `Plugin` | `chat-next-web-plugin` | 旧插件配置兼容 | 无 |
| `Sync` | `sync` | WebDAV/Upstash 同步配置 | 无 |
| `SdList` | `sd-list` | AI 绘画会话和生成记录 | 无 |
| `Update` | `chat-update` | 桌面端更新状态 | 无 |

说明：

1. `mask-store` 是历史名称，只能作为 `Skill` 的迁移来源，不再作为新版本写入基线。
2. 新版本读取 `skill-store` 为空时，会尝试读取 `mask-store` 并写回 `skill-store`。
3. `skill-store` 写入成功后会清理 `mask-store`，避免生产排查时同一类数据出现两个来源。
4. 同步快照和导入文件仍兼容旧的 `mask-store` 字段，但导出新快照时只应包含 `skill-store`。
...
1. 多模态消息保留为 `content[]`
2. 视觉输入优先走 `v1/responses`
3. `image_url.url` 可以是普通 URL，也可以是 Data URL
4. 只有模型或上游服务不支持 `v1/responses` 时，才回退到 `v1/chat/completions` 或兼容路径
